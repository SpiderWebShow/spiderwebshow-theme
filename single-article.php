...
// Function to get share counts from SharedCount.com			
/*
		function get_share_counts() {
		
			"use strict";
		
			var permalink, share_query, tw_btn, tw_num, fb_btn, fb_num, share_holder;
			
			permalink = "<?php the_permalink(); ?>";
			share_query = "//api.sharedcount.com/?url=" + permalink +'&callback=';
			
			tw_btn = $('.share-button.twitter a');
			fb_btn = $('.share-button.facebook a');
			share_holder = $('<div/>').addClass('share-holder');
			
			$.getJSON(share_query, add_share_num);

			
			// define function to add data to buttons 
			function add_share_num(data){
				
				tw_num = data.Twitter;
				fb_num = data.Facebook.total_count;
				
				if(tw_num > 0){
					tw_btn.after($('<span/>').addClass('share-holder').text(tw_num).addClass('added'));
				}
				
				if(fb_num > 0){
					fb_btn.after($('<span/>').addClass('share-holder').text(fb_num).addClass('added'));
				}
